...
struct WSActionQueueTests {
    ...
    @Test func retainedPayloadBytesEnforceHardBoundIndependentlyOfActionCount() async throws {
        let group = MultiThreadedEventLoopGroup(numberOfThreads: 1)
        let eventLoop = group.next()
        ...
        let queue = WSActionQueue(
            eventLoop: eventLoop,
            maximumOutstandingActions: 10,
            maximumOutstandingRetainedBytes: 5,
            onCapacityAvailable: { capacityAvailable.continuation.yield() }
        )

        let admitted = try await eventLoop.submit {
            [
                queue.enqueue(retainedByteCount: 3) {
                    for await _ in blocker.stream {}
                },
                queue.enqueue(retainedByteCount: 2) {},
                queue.enqueue(retainedByteCount: 1) {},
            ]
        }.get()
        let retained = try await eventLoop.submit {
            queue.outstandingRetainedByteCount
        }.get()
