...
## 이 포크에서 추가한 것 — 브라우저 클라이언트 (안드로이드 대응)

원본의 iOS 앱은 iPhone·iPad 전용입니다. **안드로이드 폰·태블릿에서 쓰려고**
relay가 웹 페이지를 직접 서빙하도록 만들고, 브라우저에서 도는 클라이언트를
붙였습니다. 앱 설치 없이 **브라우저 주소창 하나로** 끝납니다.

```
http://<내-머신-이름>:4399/      ← 같은 tailnet의 어떤 기기에서든
```

- 워크스페이스 목록 → 선택 → **실제 터미널 화면**(색·한글 그대로)
- 입력창 + 특수키 바(Esc · Tab · ↑↓←→ · Enter · Ctrl+C)
- 폰 화면이 작아도 되도록 **배율 조절**(버튼 · 두 손가락) + **화면 폭 맞춤**
- 끊기면 **스스로 재연결**하고, 토큰이 무효면 재등록까지 알아서 함

relay가 페이지와 API를 **같은 출처(origin)로** 내주는 것이 핵심입니다. 별도
웹서버를 두면 CORS에 막히고, 프록시를 거치면 Tailscale이 호출자 신원을
확인하지 못해 페어링이 깨집니다.

구현과 배포 방법은 [`web/README.md`](web/README.md)에 있습니다. relay는
`~/.cmuxremote/web`이 **존재할 때만** 정적 파일을 서빙하므로, 그 디렉터리를
만들지 않으면 기존과 똑같이 API 전용으로 동작합니다.

> iOS 앱을 쓰신다면 이 포크는 필요 없습니다. 원본을 그대로 쓰세요.
...
- 🔕 **알림 소음 줄이기** — 사용자 입력이 필요한 이벤트만 iOS 배너로 알리고, 나머지는 Inbox 기록과 안 읽음 배지로만 유지하는 설정을 추가했습니다.
- 🇰🇷 **한글/CJK 겹침 렌더링 수정** — 넓은 pane에서 한글이 인접 글자와 겹쳐 보이던 문제를 실측 폰트 메트릭 + 셀 단위 클리핑으로 수정했습니다 ([#13](https://github.com/NewTurn2017/cmux-remote/issues/13)).
- ➖ **`--` 입력 보호** — 커맨드 입력창에서 iOS 스마트 구두점이 `--`를 em dash로 바꿔 CLI 옵션이 깨지던 문제를 수정했습니다 ([#4](https://github.com/NewTurn2017/cmux-remote/issues/4)).
- 🛠️ **(relay) 셋업 호환성** — cmux 0.64+ `socket-control-password` XDG state 경로 지원, GUI Tailscale.app CLI PATH 폴백, 연결 가이드 password 안내 교정. relay는 `git pull` 후 `./scripts/install-launchd.sh` 재실행으로 업데이트하세요.
