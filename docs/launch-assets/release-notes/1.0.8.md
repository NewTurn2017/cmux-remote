# Cmux Remote 1.0.8 Release Notes

> Supersedes the never-released 1.0.6 (build 8). 1.0.8 (build 10) rolls the full
...
> before upload. Users move from 1.0.5 straight to 1.0.8.
...
- [x] Release archive + export for 1.0.8 build 10 (`aps-environment=production`, `get-task-allow=false`, `beta-reports-active=true`, App Store distribution profile; 2026-07-23).
- [x] `1.0.8 (10)` metadata re-verified for ASC field limits and URL liveness (2026-07-23).
- [ ] Build 1.0.8 (10) uploaded to App Store Connect via Transporter.
- [ ] Build selected on a new 1.0.8 version record and submitted for review.

## Notes

- 1.0.6 (build 8) was already uploaded to ASC in an earlier session and reached "Ready for Distribution", but is being abandoned rather than released so the notification-noise controls ship in the same public update. Build numbers are monotonic: 6 → (8, superseded) → (9, delivered to ASC 2026-07-05 under the 1.0.7 train) → 10 (1.0.8).
- The marketing version moved 1.0.7 → 1.0.8 because the 1.0.7 pre-release train on ASC was closed to new builds (Transporter validation 409, 2026-07-23) — build 9 had already opened that train. The public release therefore ships as 1.0.8; users move from 1.0.5 straight to 1.0.8.
