...
- Auth backend: embedded `tsnet` node vs the host's already-running `tailscaled` local API — **resolved 2026-05-10 (M3.0):** v1.0 consults the host's `tailscaled` via its local Unix socket (`/var/run/tailscaled.socket` or the macOS user-context equivalent) and calls `WhoIs(peerAddr)` to resolve the device's tailnet identity. The relay binds `0.0.0.0:4399` and relies on Tailscale ACLs / Funnel-off to keep the public out. Embedded `tsnet` is revisited in v1.2 only if Tailscale ACL push notifications become required.
